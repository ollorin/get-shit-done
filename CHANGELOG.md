...
### Fixed
- **`milestone complete` phase/plan/task counts now scoped to the milestone being closed, not every directory ever left in `.planning/phases/`** -- recurring bug reported independently across at least 4 milestones in a real consuming project's own retrospective (`igaming-platform` `.planning/RETROSPECTIVE.md`: v0.1.8 double-entry, v0.1.20 "21 phases/36 plans instead of 8/14", v0.1.28 "63 phases / 343 plans / 142 tasks" against a 8+-milestone backlog, v0.1.29 "73 phases / 389 plans / 154 tasks" instead of the actual 10/46), each time worked around by hand rather than fixed at the tool level. Root cause was two-fold: (1) `cmdMilestoneComplete`'s own stats loop walked every directory in `.planning/phases/` with no scoping at all, while the sibling `milestone summarize`/`milestone archive-phases` commands already supported a `--phases N-M` range; (2) even when an operator passed `--phases` on `milestone complete`, the CLI dispatcher's `case 'milestone': complete` branch only ever parsed `--name` before calling `cmdMilestoneComplete` -- the flag was extracted for the chained `summarize`/`archive-phases` calls but silently never reached `cmdMilestoneComplete` itself (exactly the v0.1.20 finding). Fixed both: the dispatcher now threads `--phases` into `cmdMilestoneComplete`'s own options, and a new `resolveMilestonePhaseScope` scopes the stats/accomplishments loop to (a) an explicit `--phases N-M` range when given, else (b) phase numbers parsed from ROADMAP.md's own `### Phase N: ...` headings (the current milestone's own roadmap section, per this repo's `templates/roadmap.md` convention -- this is the path that actually fixes the real-world failure, since `complete-milestone.md`'s own production invocation at line 445 never passes `--phases` at all). When neither source can determine the milestone's phase range and at least one real phase directory exists on disk, the command now exits 1 with an explicit message rather than silently falling back to counting everything -- the exact silent-wrong-answer behavior being fixed. An upstream branch (`upstream/fix/milestone-scoped-phase-counting`, commit `367149d`, PR #784 "fix/milestone-stats-scoping") already fixed the equivalent bug, but targets this fork's pre-consolidation modular `get-shit-done/bin/lib/*.cjs` layout (`milestone.cjs`) which no longer exists in this fork's current single-file `gsd-tools.js` -- the upstream diff does not apply; this fix hand-ports its `isDirInMilestone`-style approach against the current `cmdMilestoneComplete`. Output gains a new `phase_scope: {source, range}` field recording which mechanism was used. 3 new regression tests in `gsd-tools.test.js`, including one reproducing the exact real-world shape (3 different milestones' worth of un-archived phase directories coexisting on disk, matching the "old milestones never get their phase dirs archived away" trigger every prior occurrence shared) -- proven RED against the pre-fix code (7 phases/3 plans/4 tasks miscounted as the whole backlog) and GREEN after the fix.
- **Phase 59-04: coordinator DEPENDS_ON contract_mismatch fix (MILE-38 gap closure)** -- `coordinator-detail.md`'s `cross_phase_integration` step computed `DEPENDS_ON` via `roadmap get-phase {phase_number} --raw` piped into a `JSON.parse`, but `--raw` on this subcommand intentionally prints raw markdown section text (relied on by `planner-detail.md`/`workflows/verify-phase.md`, both left untouched), so `JSON.parse` always threw and `DEPENDS_ON` silently resolved to `[]` regardless of real ROADMAP.md content -- defeating both the pre-existing step-3 explicit-dependency trigger and 59-03's new step 3b declared-dependency `gsd-integration-tester` spawn. Fixed by dropping `--raw` from this one invocation (verified: `roadmap get-phase 59` now returns `depends_on: ["Phase 55"]` end-to-end through the exact bash line in the prose file, not a hand-rolled equivalent). A new regression test executes the LITERAL extracted bash snippet against fixture ROADMAP.md files (declared-dependency and independent-phase cases) and a grep-assertion permanently locks out any future `roadmap get-phase` + `--raw` + `JSON.parse` recombination in this step. Found by `gsd-integration-tester` during Phase 59 verification.
...
### Added
- **`GSD_AUTONOMOUS=1` — unattended roadmap resumption after a supervisor relaunch** -- `workflows/execute-roadmap.md`'s two remaining human-approval prompts (`resume_capability`'s resume/restart/stop choice, `confirm_execution`'s yes/skip/stop choice) now auto-proceed WITHOUT blocking when both `GSD_AUTONOMOUS=1` is set in the environment AND `resume_state` is present (a prior incomplete execution exists) -- `resume_capability` auto-selects "resume" (NEVER "restart" -- that discards completed-phase history and would be catastrophic to auto-select unattended), and `confirm_execution` auto-proceeds as "yes" on the reasoning that the human already authorized this roadmap when they originally started it. A **fresh** run (no `resume_state`) is never auto-started this way regardless of `GSD_AUTONOMOUS` -- a human must still approve work nobody has authorized yet. Every auto-decision is logged via a new `autonomous_resume_auto_selected` / `autonomous_confirm_auto_proceeded` execution-log event, never silently answered. This exists because an external process supervisor (wrapping `claude -p "/gsd:execute-roadmap"`, detecting a session/quota death of the whole process, waiting for reset, and relaunching a brand-new headless process) has no human present across the relaunch boundary -- without this, the fresh headless process stalls indefinitely at the first prompt, or the model improvises an answer nothing authorized. A new `<autonomous_resumption>` section in `execute-roadmap.md` documents the supervisor invocation pattern and the exact two-gate contract. `references/resilience.md` also gains an explicit scope-boundary note: its sleep-loop/wait-until-reset recovery machinery is prose executed BY a live LLM orchestrator and structurally cannot fire when the orchestrator process itself is what died -- only an external supervisor survives that case; earlier phrasing there implied otherwise.
- **Phase 61: project-aware pre-PR gate (MILE-41 complete, Plans 61-01/61-02/61-03)** -- The pre-PR gate no longer hardcodes a Deno/Nx/Supabase check set. Plan 61-01: new `get-shit-done/config/pre-pr-checks.json` (flat, config-as-data, mirroring `model-registry.json`) + fail-open `get-shit-done/bin/pre-pr-checks.js` accessor (`detectProjectTypes`, `checksForNode`, `discoverMakeTestCommand`, `deriveCheckSet`) -- detection reads ONLY manifest file presence (`package.json`/`pyproject.toml`/`go.mod`/`Cargo.toml`), never manifest content, and for node ONLY the scripts a project actually declares (`test`/`lint`/`build`) become checks, never synthesized. Plan 61-02: `cmdGatePrePr` now derives its `checks` array via `deriveCheckSet(cwd)`, preserving the exact `{gate,passed,action_required,checks,instructions,mark_command}` output contract `execute-roadmap.md` already parses (locked by a grep-assertion regression test), with the 3 pre-existing branches (`--mark-passed`, incomplete-phases, cached-marker) completely unchanged; unknown project types degrade to a minimal universal check set (git-status-clean, branch-not-main) with `degraded:true` + a loud human-readable notice, exit 0, never a crash. Plan 61-03: a permanent, deterministic regression test proves the real GSD repo root itself resolves to `detected_types: ['node']` and exactly one derived check (`npm run test` -- the repo's only declared script), and a real, full `npm test` run captured as evidence confirms that derived check genuinely passes. 905/905 tests passing (was 904 at phase start). MILE-41 (Project-Aware Pre-PR Gate) satisfied end-to-end -- v1.15.0 (Self-Improving Quality Loop) now COMPLETE across all 8 phases (54-61).
- **Phase 61-01: project-aware pre-PR gate foundation -- detection + check-set derivation module (MILE-41, foundation)** -- new `get-shit-done/config/pre-pr-checks.json` (config-as-data check registry keyed by project type: `node`/`python`/`go`/`rust`/`unknown`, mirroring `model-registry.json`'s precedent exactly) and `get-shit-done/bin/pre-pr-checks.js`, a fail-open accessor module for the pre-PR gate `gsd-tools.js`'s `cmdGatePrePr` will consume in Plan 61-02. `loadRegistry()` mirrors `model-registry.js`'s `loadRegistry()` shape/behavior byte-for-byte: missing config file, corrupt JSON, or JSON missing any of the 5 required top-level keys all fall back wholesale to the in-module `DEFAULT_CHECKS` constant (value-identical to the JSON), never throwing. `detectProjectTypes(cwd)` is a PURE function inspecting manifest file PRESENCE only (`package.json`/`pyproject.toml`/`go.mod`/`Cargo.toml`) in fixed `['node','python','go','rust']` order, returning the union of every matching type (never first-match-only, never invented types) -- a per-manifest try/catch means one unreadable manifest never blocks checking the others. `checksForNode(cwd, typeConfig)` derives node checks ONLY from scripts actually declared in `package.json`'s `scripts` object (via `getDeclaredNodeScripts`, itself fail-open on malformed JSON) -- a project with only a `test` script produces exactly one `node-test` check, never a synthesized `lint`/`build` command. `discoverMakeTestCommand(cwd)` is a best-effort, never-throwing helper that appends a `make-test-discovered` check to the degraded path when a `Makefile` with a `test:` target is found. `deriveCheckSet(cwd)` is the main entry point: known project type(s) resolve to the union of each type's derived checks (`degraded:false`); zero detected manifests return the minimal universal check set (`git-status-clean`, `branch-not-main`) plus any make-test discovery, with `degraded:true` and a non-empty human-readable notice -- never a crash, never an empty/uninformative result. Verified end-to-end against this repo's own real `package.json` (which declares only `test`): `deriveCheckSet(process.cwd())` returns exactly one `node-test` check with `degraded:false`, proving the module already self-hosts correctly. 21 new tests in `pre-pr-checks.test.js` (mirroring `model-registry.test.js`'s isolated-module copy-into-temp-dir strategy) cover fail-open (missing/corrupt/missing-key config, valid-config-used-as-is), per-type detection (node/python/go/rust/multi-manifest-union/none/unreadable-manifest-never-throws), node declared-scripts-only derivation (partial declaration, zero scripts, malformed `package.json`), and full `deriveCheckSet` behavior (known type, multi-manifest union, degraded/unknown with notice, Makefile discovery with/without a `test:` target, nonexistent-cwd never-throws). Wired into `package.json`'s `scripts.test` chain; 896/896 tests passing (was 875 at plan start). This plan lands the pure detection/derivation module only -- Plan 61-02 wires it into `gsd-tools.js`'s `cmdGatePrePr`.
- **Phase 60: adversarial plan review (MILE-39 complete, Plans 60-01/60-02/60-03)** -- High-risk plans (explicit `high_risk: true` frontmatter, or config-gated criteria: file-count threshold, security-sensitive file pattern, 3+ `tdd="true"` tasks) now get reviewed by a three-agent attacker/defender/judge trio instead of the single-pass `gsd-plan-checker`. Plan 60-01: `loadConfig` gains `quality.adversarial_review`/`adversarial_review_file_threshold`/`adversarial_review_security_patterns` (all default off/zero-behavior-change), `computeHighRisk` (explicit frontmatter flag always wins even with the toggle off), `computePresentationOrder` (a deterministic content-hash, NOT `Math.random()`, so attack/defense ordering is reproducible), and `verdictToIssues` (converts a judge verdict into the exact `issues:` shape `gsd-plan-checker` already produces) back two new CLI subcommands: `quality assess-risk`/`quality verdict-to-issues`. Plan 60-02: three new agents -- `gsd-plan-attacker` (read-only, finds flaws), `gsd-plan-defender` (read-only, rebuts strictly from cited plan/codebase evidence), `gsd-plan-judge` (rules on every flaw, writes its own `{plan_id}-VERDICT.md` durable artifact, mirroring how `gsd-verifier` writes `VERIFICATION.md`) -- each carrying the house `<content_firewall>` + MILE-26 Telemetry convention. Plan 60-03: `plan-phase.md` gains a new Step 9.5 risk-triage that decides `TRIAGE_MODE` (trio vs checker) per phase; Step 10 branches accordingly, with the EXISTING `gsd-plan-checker` spawn preserved byte-identical inside the checker branch (criterion 3: non-high-risk plans are unaffected); Step 11 converts trio verdicts into the checker's issues shape and feeds the EXISTING Step 12 revision loop unchanged; a fail-open guard falls back to the standard checker for the whole phase if any trio member errors. `budget check --raw` still `pass:true` for all 5 budget-measured agents; 875/875 tests passing (was 820 at phase start). MILE-39 (Adversarial Plan Review) satisfied end-to-end -- v1.15.0 Phase 60 complete.
- **Phase 60-02: adversarial-review agent trio (MILE-39)** -- three new agent files complete the attacker/defender/judge persona set `plan-phase.md`'s risk-triage step (Plan 60-03) will spawn for high-risk plans. `agents/gsd-plan-attacker.md` (`Read, Grep, Glob, Bash` -- no Write/Edit) finds concrete flaws (logical fallacies, missed edge cases, missed dependencies, untestable success criteria), each tagged with a severity (`critical`/`major`/`minor`) and a specific plan/task reference, returning a structured `flaws:` list. `agents/gsd-plan-defender.md` (same read-only tool set) rebuts each flaw strictly from cited evidence -- exact plan text, a task ID, a `must_haves` entry, or a real codebase file/line -- ruling each `refuted`/`conceded`/`partially-conceded`, never inventing facts. `agents/gsd-plan-judge.md` (`Read, Write, Bash, Grep, Glob` -- the only one with Write) reads whichever of `attacker_flaws`/`defender_rebuttals` its `presentation_order` input names first, independently re-verifies every flaw rather than adopting the defender's ruling, rules each `dismissed`/`valid-minor`/`valid-major`/`valid-critical`, derives one overall verdict (`approved`/`revise`/`critical`), and writes its own durable `{phase_dir}/{plan_id}-VERDICT.md` artifact -- mirroring how `gsd-verifier` writes its own `VERIFICATION.md`. All three carry the house `<content_firewall>` pointer and the MILE-26 4-field Telemetry self-report line, matching `gsd-test-writer.md`/`gsd-integration-tester.md`'s established convention exactly. A new `describe('Phase 60-02: ...')` block in `gsd-tools.test.js` parses all 3 files' frontmatter directly via `gray-matter` against the real files on disk, asserting well-formed `name`/`description`/`tools`, the exact `content_firewall`/Telemetry substrings, the attacker/defender Write/Edit exclusion vs. the judge's Write inclusion, and each agent's distinct output contract (`flaws:`/`rebuttals:`/`VERDICT.md`) -- proving none is a copy-pasted `gsd-plan-checker` clone. 865/865 tests passing (was 850). This plan lands the three agent files only -- Plan 60-03 wires `plan-phase.md`'s risk-triage step to spawn them.
- **Phase 60-01: adversarial-review config + risk-assessment CLI (MILE-39, foundation)** -- `loadConfig` gains `quality.adversarial_review`/flat `adversarial_review_enabled` (default `false`), `quality.adversarial_review_file_threshold`/flat `adversarial_review_file_threshold` (default `8`), and `quality.adversarial_review_security_patterns`/flat `adversarial_review_security_patterns` (default 10-entry array), following the exact flat+nested pattern established by `test_writer_enabled`/`quality.test_writer`. New pure functions `computeHighRisk` (explicit `high_risk: true` frontmatter always wins even when the toggle is off; otherwise gated on the toggle, criteria: files_modified count, security-pattern substring match, tdd="true" task count) and `computePresentationOrder` (a deterministic content-hash, NOT `Math.random()`, so attack/defense ordering is reproducible for testing) back a new `quality assess-risk <plan-file> [--raw]` CLI. New `verdictToIssues` pure function + `quality verdict-to-issues <verdict-file> [--raw]` CLI converts a judge's verdict frontmatter into the exact `issues:` shape `gsd-plan-checker` already produces, so the existing plan-revision loop can consume adversarial-review verdicts with zero new revision mechanism.
- **Phase 59: dormant quality agents wired in (MILE-37/MILE-38 complete, Plans 59-01/59-02/59-03)** -- gsd-test-writer and gsd-integration-tester, previously dormant satellite agents, now join the golden path behind two new default-off config toggles (`quality.test_writer`/`quality.integration_tester`). Plan 59-02: the executor spawns gsd-test-writer after every source-touching `type="auto"` task (not just `tdd="true"` tasks) when `quality.test_writer` is on, treating 0-tests/missing output as a loud `[Rule Quality-TW]` SUMMARY.md deviation rather than a silent skip or a hard block -- the existing `tdd="true"` hard-block contract is completely unchanged. Plan 59-03: the coordinator spawns gsd-integration-tester on a phase with a declared ROADMAP.md `depends_on`, independent of the pre-existing file-overlap heuristic, when `quality.integration_tester` is on (never for an independent phase); a blocking contract mismatch is propagated into the phase's own VERIFICATION.md via a new `verify append-gap` CLI, composing with (not duplicating) the existing Phase 55 `eval-candidate from-verification` pipeline. Also fixed a real, load-bearing pre-existing bug: `roadmap get-phase` never actually returned a `depends_on` field, so the coordinator's "explicit dependency" trigger condition had silently never functioned against the real ROADMAP.md. A new `tests/fixtures/eval-project/golden-artifacts/post-59/` fixture proves both new spawns via `eval-harness.js`'s existing `assertAgentsSpawned`. 816/816 tests passing (was 801 at phase start); `budget check --raw` still `pass:true` for all 5 agents.
- **Phase 59-02: gsd-test-writer joins the golden path after every source-touching task (MILE-37)** -- `gsd-tools.js` gains `isSourceFile`/`isTestOrSpecFile`/`computeTouchesSourceCode`, pure functions (no I/O) mirroring `isUIFile`/`computeHasUI`'s exact extension+path-pattern style (Phase 45-02): a file is "source" if it carries one of 7 recognized extensions (`.js`/`.ts`/`.tsx`/`.jsx`/`.py`/`.go`/`.rb`) AND is not itself a test/spec file (`*.test.*`/`*.spec.*` or anything under `test/`/`tests/`/`__tests__/`/`spec/`). A new top-level `quality touches-source --files <comma-list>` CLI thinly wraps it. `get-shit-done/references/executor-detail.md` gains a new, ADDITIVE `<post_task_quality_spawn>` block (positioned after `<inter_task_syntax_check>`, before the existing `<test_task_handling>` -- which is byte-for-byte unchanged) that spawns `gsd-test-writer` after every `type="auto"` task (not itself a `tdd="true"` task) that touches source code, gated on the new `quality.test_writer` config toggle (default off -- zero behavior change). Unlike the `tdd="true"` contract's hard block, a 0-tests/missing-output result here is a LOUD, greppable `[Rule Quality-TW]` SUMMARY.md Deviation -- the plan always continues, never blocks. `[Rule 1 fix]` discovered while writing the toggle-off regression test: the prose's `config get test_writer_enabled` bash snippet was missing `--raw`, so the shell comparison against `"true"` could never match even when the config genuinely enabled the toggle -- fixed by adding `--raw`, with a dedicated regression test locking in the corrected invocation. 19 new tests (pure-function unit tests across all required categories, real-CLI integration tests, index-bounded grep-assertion tests locking the new block's position/content, a regression guard proving `<test_task_handling>`'s existing hard-block contract is untouched, and the toggle-off/`--raw` regression proof); `budget check --raw` still `pass:true` for all 5 agents (`executor-detail.md` is not budget-measured); 800/800 tests passing (was 781). MILE-37 satisfied end-to-end; MILE-38 (coordinator/gsd-integration-tester wiring) remains open for Plan 59-03.
- **Phase 59-01: quality-agent config toggles + gap-propagation foundation (MILE-37/MILE-38, foundation)** -- `loadConfig` gains `quality.test_writer`/flat `test_writer_enabled` and `quality.integration_tester`/flat `integration_tester_enabled`, both defaulting to `false` (zero behavior change until explicitly enabled), following the exact flat+nested pattern established by `routing_min_sample_count`/`savings_baseline_profile`. New `appendVerificationGap`/`verify append-gap` CLI composes with the existing `eval-candidate from-verification` reader (Phase 55) rather than a second gap pipeline -- a blocking cross-phase mismatch (Plan 59-03) appends one gap entry and flips `status: gaps_found` on the phase's own VERIFICATION.md. `agents/gsd-test-writer.md`/`agents/gsd-integration-tester.md` (about to join the golden path) both gain a `<content_firewall>` block and the MILE-26 4-field telemetry self-report line.
- **Phase 58: honest token accounting (MILE-36 complete, Plans 58-01/58-02/58-03)** -- Plan 58-01: new `get-shit-done/bin/token-usage-ledger.js`, a durable per-task usage record mirroring the MILE-26 Self-Report Telemetry pattern: `appendTaskUsage`/`readTaskUsageRecords` are a thin-I/O JSONL append/read pair against `.planning/telemetry/token-usage.jsonl` (never throws, malformed lines skipped with a warning), plus four pure functions -- `estimateTaskTokens` (crude, honest per-tier heuristic), `resolveBaselineTier` (maps a CONFIGURED baseline profile to a comparison tier -- never a hardcoded all-Opus assumption), `computeSavingsFromUsage` (coverage-aware savings that explicitly distinguishes no-data/partial/full coverage and per-record `source:'estimated'` vs real counts), and `formatUsageSavingsTable` (renders the honesty labels, never overclaims); `loadConfig` gains a `savings_baseline_profile` key (default `'quality'`, flat key + `savings.baseline_profile` nested fallback). Plan 58-02: new `token-usage record --phase N --plan X --task-index N --task-name "..." --tier T` CLI subcommand (`cmdTokenUsage` -- a NEW namespace, deliberately distinct from the existing `token` TokenBudgetMonitor command), defaulting to `estimateTaskTokens` when explicit token counts are absent so every golden-path record is labeled `source:'estimated'`, honestly; `savings report` is retargeted from the old `token_budget.json`-based `generateReport()` -- a hardcoded all-Opus baseline computed against a file NOTHING ever populated -- onto the recorded-usage ledger via `readTaskUsageRecords` + `computeSavingsFromUsage` + `formatUsageSavingsTable`, with explicit empty/partial/full coverage reporting (`savings calculate` untouched). Plan 58-03: the golden-path wiring itself -- `coordinator-detail.md`'s `<step name="execute">` PER_TASK_MODE step 7 now calls `token-usage record` (best-effort, never blocking) immediately after each of its 3 `task_outcome` logging call sites (first-attempt success, per-attempt failure inside the bounded escalation loop, final success after re-spawn), reusing the same in-scope variables and adding NO new field to the executor's return contract -- closing the exact "zero call sites" gap 58-RESEARCH.md's audit found for the old `token` namespace. Coverage: 25 ledger/config unit tests (`token-usage-ledger.test.js`), 6 real-CLI subprocess integration tests against isolated temp projects (`token-usage-cli.test.js`), and 5 grep-assertion wiring tests in `gsd-tools.test.js` (presence/count/adjacency of all 3 call sites inside the execute step, no `--tokens-input` in golden-path calls, `checkAllBudgets` regression) -- 762/762 tests passing (was 726 at phase start). MILE-36 (Honest Token Accounting) now satisfied end-to-end: savings claims are computed from real recorded usage (labeled ESTIMATED, since no harness mechanism yet exposes real per-spawn token counts), with explicit coverage reporting and a configured (never hardcoded all-Opus) baseline profile. Phase 58 complete across all 3 plans.
- **Phase 57-04: bounded coordinator escalation-loop wiring + ledger recording (MILE-35 complete)** -- `get-shit-done/references/coordinator-detail.md`'s `PER_TASK_MODE` escalation block (previously a haiku-only, unconditional, single-hop escalation) is replaced with the full bounded loop: on every executor return the coordinator derives a `task_type` (`routing task-type`, best-effort), logs exactly one `task_outcome` event for every terminal attempt (success or failure), and on a `"TASK FAILED:"` signal calls `routing escalation-decision` in a loop -- honoring the executor's own `[non-capability]` tag as authoritative (never re-derived), logging one `tier_escalation` event per hop, re-spawning the same task at the next tier, and terminating on success, ladder exhaustion, the explicit escalation bound, or a non-capability classification. A new final item in the `execute` step runs `routing ledger build --raw` once after all plans/waves in the phase complete (before `post_phase_ux_sweep`), folding the phase's `task_outcome`/`tier_escalation` events into `.planning/routing-ledger.json` -- best-effort, never blocking. Since `coordinator-detail.md` is prose consumed by an LLM subagent, coverage is split: 4 new integration tests simulate the full haiku->sonnet->opus chain, bound enforcement, non-capability exclusion, and ledger recording by driving the REAL `routing escalation-decision`/`execution-log event`/`routing ledger build` CLI against an isolated temp project; 7 grep-assertion tests index-lock the prose wiring (all three calls inside `<step name="execute">`, `routing ledger build` before `post_phase_ux_sweep`, no second hardcoded ladder). `budget check --raw` still reports `pass:true` for all 5 agents (coordinator-detail.md is not budget-measured). 726/726 tests passing (was 715). MILE-35 (bounded tier escalation) now satisfied end-to-end across Plans 57-03/57-04; Phase 57 (Outcome-Informed Routing Ledger & Bounded Escalation) is now COMPLETE across all 4 plans -- MILE-34/MILE-35 both satisfied, all 8 required integration-test scenarios covered across the phase.
- **Phase 57-03: bounded-escalation decision logic + executor failure classification (MILE-35, decision-logic half)** -- `gsd-tools.js` gains `classifyFailure(errorSummary)`, a pure function matching a fixed 15-entry, case-insensitive deny-list (`ENOENT`, `no such file`, `command not found`, `permission denied`, `EACCES`, `ECONNREFUSED`, `getaddrinfo`, `network error`, `env var`, `environment variable`, `.env`, `ENOSPC`, `disk full`, `git conflict`, `merge conflict`) that returns `{capability_related: false, matched_pattern}` on any match and `{capability_related: true, matched_pattern: null}` otherwise (the safer default for a self-healing system); and `decideEscalation(currentTier, classification, escalationsUsed)`, a pure ladder-bounded decision function reusing `model-registry.js`'s `getNextTier`/`getTiers` (never a second hardcoded ladder) -- non-capability failures never escalate, an explicit `escalationsUsed >= getTiers().length - 1` bound stops runaway retries independent of the ladder's own `opus -> null` termination, and the opus ceiling itself never attempts a 4th tier. Two new `routing classify-failure "<error text>"` / `routing escalation-decision --current-tier X --error-summary "..." --escalations-used N` CLI subcommands extend the existing `case 'routing':` dispatch, the latter composing `classifyFailure` + `decideEscalation` into one call the coordinator can use. `get-shit-done/references/executor-detail.md`'s failure-signaling block now calls `routing classify-failure` before emitting a `TASK FAILED` signal and appends a `[non-capability]` marker only when `capability_related: false` -- the original unmarked format string is preserved byte-for-byte for capability-related failures, so any existing consumer parsing the old format is unaffected. 36 new tests (15 deny-list pattern tests plus case-insensitivity/capability-shaped/empty/undefined-input coverage, 7 `decideEscalation` bound/ladder scenarios, 5 CLI integration/composition tests, 4 `executor-detail.md` grep-assertion/regression-guard tests); 715/715 tests passing (was 679). This plan lands the pure decision logic only -- MILE-35 remains open for Plan 57-04's coordinator-loop wiring.
- **Phase 57-02: routing ledger consultation & task-router wiring (MILE-34 complete)** -- `gsd-tools.js` gains `consultLedger(ledger, taskType, heuristicTier, minSampleCount)`, a pure decision function reusing `model-registry.js`'s `getNextTier` for a single-hop-up-only tier adjustment: a null ledger fails open (`{tier: heuristicTier, adjusted: false, fail_open: true, reason: 'ledger_unavailable'}`); no bucket for the task type or total samples below the minimum count leave the heuristic untouched (`reason: 'no_data_for_task_type'`/`'insufficient_sample'` -- the sub-minimum type is IGNORED even when the raw rates would otherwise justify an adjustment); a genuine contradiction (heuristic tier `success_rate < 0.5` AND the next tier up has `attempts >= minSampleCount` AND `success_rate >= heuristic_rate + 0.2`) adjusts UP exactly one tier; the function never adjusts down and never adjusts past the opus ceiling. A new `routing ledger consult --task-type X --heuristic-tier Y --raw` CLI subcommand composes `readRoutingLedger` + `consultLedger`, printing a loud `WARNING: routing ledger unavailable (...) -- falling back to heuristic-only routing` to stderr on any fail-open path while still exiting 0 and returning the decision JSON on stdout. `agents/gsd-task-router.md` gains a new `consult_ledger` step positioned between `check_quota` and `get_context` (runs `routing task-type` then `routing ledger consult`, overriding the tier on `adjusted: true`), and both `return_decision` output templates now carry an unconditional `Ledger:` status line -- the task router demonstrably reads the ledger and can override its heuristic tier, satisfying MILE-34's full acceptance criterion. 20 new tests (7 pure-function unit tests across all required categories, 5 CLI integration tests including absent-ledger and corrupt-ledger fail-open scenarios, 4 grep-assertion tests locking in `gsd-task-router.md`'s step ordering and output-template wiring); 677/677 tests passing (was 662). MILE-34 (Outcome-Informed Routing Ledger) now satisfied end-to-end across both plans 57-01/57-02; MILE-35 (bounded tier escalation) remains open for Plans 57-03/57-04.
- **Phase 57-01: routing ledger storage & build aggregation (MILE-34, storage half)** -- `execution-log.js`'s `EVENT_TYPES` gains additive `TASK_OUTCOME`/`TIER_ESCALATION` keys (non-breaking; `appendEvent` never validated against the enum). `gsd-tools.js` gains `deriveTaskType(taskDescription)` (buckets any task description into a fixed ordered 20-verb vocabulary via first whole-word match, falling back to `'other'`), `buildRoutingLedger(events, options)` (pure aggregator bucketing `task_outcome` events by `(task_type, tier)` into `{attempts, successes, failures}`, with optional best-effort telemetry `quality_flags` enrichment keyed on matching `phase`+`plan`), and `readRoutingLedger`/`writeRoutingLedger` (fail-open persistence for a new rebuildable `.planning/routing-ledger.json`, mirroring `readDeferredWaivers`'s exact `{ok, ledger, reason}` contract -- missing file is not an error, corrupt file always surfaces loudly, never throws). New `routing task-type "<desc>"` / `routing ledger build` / `routing ledger show` CLI subcommands extend the existing `case 'routing':` dispatch (no new top-level command); `loadConfig` gains `routing_min_sample_count` (default 5, overridable via `.planning/config.json`'s `routing.min_sample_count`). 22 new tests covering derivation ordering/whole-word matching, ledger aggregation, fail-open read/write round-trips, config defaults, and real-CLI integration against temp-project fixtures (including a corrupt-ledger `show` scenario). 660/660 tests passing (was 638). Lays the storage foundation Plan 57-02 builds ledger consultation on top of.
- **Phase 56-03: MILE-33 end-to-end integration tests + full-suite gate (MILE-33 complete)** -- 8 cross-cutting integration tests added to `gsd-tools.test.js` (`describe('MILE-33 end-to-end: reflective prompt optimization (Phase 56)')`), mirroring 55-03's MILE-32 pattern exactly: each test builds its own isolated temp project (fixture `agents/*.md`, temp `.planning/telemetry/agent-reports.jsonl`, temp `tests/eval-regressions/accepted/`, temp `get-shit-done/config/prompt-budgets.json`) and drives the real `prompt-optimize --agent <name>` CLI subprocess via `runGsdTools`, never touching the real repo's `agents/` or `.planning/` trees. Covers all 5 required MILE-33 scenarios: (a) diagnosis generation from seeded telemetry ambiguities produces `ready_for_review` with a `diagnosis.md`/`candidate.diff` pair on disk naming the agent and containing the seeded ambiguity text; (b) a deliberately tiny configured budget rejects the candidate with `reason: budget_exceeded`, still writing `diagnosis-rejected.md`/`candidate-rejected.diff` for audit; (c) an accepted eval candidate whose `file_contains` assertion the revision breaks rejects with `reason: eval_failed` while the budget gate independently passes, isolating the eval gate; (d) a fixture agent with zero telemetry/eval signal exits 0 with `status: no_signal` and writes nothing to disk at all; (e) the real written `candidate.diff` file (not a unit-level `isValidUnifiedDiff` call) is asserted to contain valid `--- `/`+++ `/`@@ ... @@` unified-diff structure. `package.json`'s `scripts.test` was already wired to include `prompt-optimize.test.js` by 56-02's deviation fix -- no further change needed here. A final real-repo smoke test (`prompt-optimize --agent gsd-test-writer` against the live repo root) confirms exit 0 (`no_signal`, never an unhandled exception) and `git status` before/after is byte-identical -- the never-auto-apply guarantee holds against the live repo, not just temp fixtures. 638/638 tests passing (was 630). Phase 56 (Reflective Prompt Optimization) now COMPLETE across all 3 plans -- MILE-33 satisfied end-to-end.
- **Phase 56-02: reflective prompt-optimization gating + orchestration + CLI (MILE-33)** -- `get-shit-done/bin/prompt-optimize.js` gains the deterministic gating layer built on top of Plan 56-01's pure core: `checkBudgetForCandidate` measures an in-memory candidate revision against `config/prompt-budgets.json` and fails OPEN (`pass:true` + warning) when the config is missing/malformed or the target agent has no configured budget entry, failing CLOSED only on a genuine overage; `checkEvalForCandidate`/`executeEvalCandidateAgainstContent` scope accepted eval candidates to exactly the target agent's `relPath` and execute each assertion entirely in-memory against the revised content (never the real file on disk -- `file_exists`/`file_not_exists` trivially pass since the target is already known to exist, `file_contains`/`file_not_contains` check the revised string directly); `writeReviewArtifacts` is the ONLY write path in the whole feature, writing `diagnosis.md`/`candidate.diff` (or `diagnosis-rejected.md`/`candidate-rejected.diff` for rejected candidates, kept for audit) under `.planning/prompt-optimize/{agentName}/{timestamp}/`; and `runPromptOptimize` composes all of it into one of four exact outcomes -- `error`, `no_signal` (nothing written), `rejected` (`budget_exceeded`|`eval_failed`, review artifacts still written), or `ready_for_review`. `gsd-tools.js` gains a new top-level `prompt-optimize --agent <name>` CLI subcommand (`cmdPromptOptimize`), direct-exit like `eval regress`/`budget check` (exit 0 for `no_signal`/`ready_for_review`, exit 1 for `rejected`/`error`) -- `agents/*.md` is never written to by any code path. 17 new tests across all 6 TDD categories in `prompt-optimize.test.js` (now 33 total), including real CLI subprocess tests proving the exit-code contract and a proof that the no-signal path writes nothing to disk. Also fixed a gap from Plan 56-01: `prompt-optimize.test.js` had never been wired into `package.json`'s `test` script, so its tests were silently excluded from `npm test`/CI -- now included. 630/630 tests passing (was 597).
- **Phase 56-01: reflective prompt-optimization core -- deterministic diagnosis + hand-rolled diff engine (MILE-33)** -- new `get-shit-done/bin/prompt-optimize.js`, 8 pure, fail-safe, LLM-free functions forming the read/diagnose/diff half of MILE-33: `resolveAgentFile` (resolves a bare name, full name, or explicit path to a real `agents/*.md` file, `{error}` on no match, never throws), `readTelemetryForAgent`/`readEvalFailuresForAgent` (filter `.planning/telemetry/agent-reports.jsonl` and `tests/eval-regressions/{queue,accepted}/` to one agent's signal, reusing `eval-harness.js`'s `loadAcceptedEvalCandidates`/`executeEvalCandidate` -- only currently-failing accepted regressions count, with a documented `context.agent`-exact-match-first, substring-heuristic-fallback agent-association rule), `hasSignal`, `buildDiagnosis` (template-based, deterministic -- byte-identical output for identical input, `topIssue` priority: ambiguity \> instructions_not_followed.why \> eval-failure \> fallback string), and a hand-rolled LCS-based `computeUnifiedDiff`/`isValidUnifiedDiff`/`buildRevisionCandidate` (no new npm dependency) that inserts an auto-suggested clarification line immediately before `CORE_PREAMBLE_MARKER` so it counts against the budget-checked preamble. `get-shit-done/bin/prompt-budget.js` gains an additive `measurePreambleFromContent(content)` extraction -- `measurePreamble(filePath)` is now a thin wrapper delegating to it, zero behavior change to the existing file-based API. 18 new tests (16 in `prompt-optimize.test.js` across all 6 TDD categories, 2 additive in `prompt-budget.test.js` proving content/file measurement parity); 597/597 tests passing (was 595). Foundation for Plan 56-02's budget/eval gating and CLI orchestration.
- **Phase 55-03: eval-regression loader/executor + CI wiring -- accepted candidates become permanent, continuously-run eval cases (MILE-32)** -- `get-shit-done/bin/eval-harness.js` gains four additive exports: `validateEvalCandidateShape` (pure, never-throws, returns a bare error-string array -- deliberately duplicating 55-02's `validateEvalCandidateSchema` rules in miniature rather than importing them, since `gsd-tools.js` already requires this file and the reverse would be circular), `loadAcceptedEvalCandidates` (reads every `*.json` under an accepted/ dir in sorted order; a file that fails JSON parsing or shape validation is reported as an explicit `valid:false` results entry -- never silently dropped -- a documented second deliberate deviation from the file's fail-safe convention, because a malformed COMMITTED regression fixture is a real CI/data bug, not an expected missing-data case), `executeEvalCandidate` (runs one candidate's `expected` assertion -- `file_exists`/`file_not_exists`/`file_contains`/`file_not_contains` -- against the live repo, always returning `{id, pass, reason}`, never throwing), and `runEvalRegressions` (the aggregate entry point; an empty or absent accepted/ dir is the explicit trivial pass `{pass:true, total:0}`). `gsd-tools.js` gains an `eval regress <accepted-dir> --project-root <dir>` subcommand extending 53-01's `case 'eval':` dispatch, direct-exit like `eval assert` (exit 0 on pass, 1 on any failing assertion or malformed file). `npm test` now chains `eval regress tests/eval-regressions/accepted --project-root .`, and `.github/workflows/eval-harness.yml` adds `tests/eval-regressions/**` to its push/pull_request path filters plus a dedicated `eval-regress` job -- closing the MILE-32 loop: a candidate accepted via 55-02's review queue is now a permanent eval case re-verified on every push/PR. 17 new tests: 10 loader/executor tests in `eval-harness.test.js` (happy path, malformed-JSON loudness, schema-invalid file, absent dir, empty-dir trivial-pass contract, a real `eval regress` CLI subprocess run with non-zero exit + reasoned failure, and an additive-`module.exports`-growth regression guard) plus 7 cross-cutting integration tests in `gsd-tools.test.js` proving all 4 MILE-32 success-criterion scenarios end-to-end via the real CLI (candidate generation from a debug session, accept/reject review-queue transitions, CI pickup of an accepted fixture flipping pass-\>fail when the target regresses, and loud non-crashing handling of a malformed committed candidate). 595/595 tests passing (was 578).
