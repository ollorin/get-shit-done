# gsd-verifier.md — Verification Process Detail (Phase 53-02, MILE-30)

This file holds the FULL, VERBATIM `<verification_process>` block (Steps 0
through 8f: previous-verification check, must-haves establishment, observable
truths, three-level artifact verification, key-link/wiring verification,
done-criteria traceability, requirements coverage, test-content/hollow-test
detection, PRD intent alignment, anti-pattern scanning, human-verification
identification, runtime test suite execution, Charlotte QA coverage,
deferral-language detection, E2E coverage, implementation-file test
coverage, migration timestamp conflicts, docs validation), plus the
`<stub_detection_patterns>` pointer and the full `<kb_feedback>` (Step 11,
anti-pattern KB writes) -- relocated from `agents/gsd-verifier.md` during
the MILE-30 hard-rules-first restructuring. Nothing here was reworded,
summarized, or deleted.

The core preamble of `gsd-verifier.md` keeps `<core_principle>`,
`<critical_rules>`, the full `<output>` return contract, `<success_criteria>`,
and a `<hard_rules_digest>` of verbatim "Hard rule:"/"NEVER a warning"/
"CRITICAL" excerpts (including the full Step 9 Determine Overall Status
section) pulled from this detail so they surface early in the prompt. Those
