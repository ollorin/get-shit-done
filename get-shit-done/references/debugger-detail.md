# gsd-debugger.md — Investigation Methodology Detail (Phase 53-02, MILE-30)

This file holds the FULL, VERBATIM debugging methodology relocated from
`agents/gsd-debugger.md` during the MILE-30 hard-rules-first restructuring:
philosophy (user-as-reporter, meta-debugging your own code, foundation
principles, cognitive biases, systematic investigation disciplines, when to
restart), hypothesis testing (falsifiability, forming hypotheses,
experimental design, evidence quality, decision points, recovery from wrong
hypotheses, multiple hypotheses strategy), investigation techniques (binary
search, rubber duck, minimal reproduction, working backwards, differential
debugging, observability first, comment-out-everything, git bisect,
technique selection/combination), verification patterns (what "verified"
means, reproduction/regression/environment/stability verification,
test-first debugging, verification checklist, red flags, mindset),
research-vs-reasoning guidance, and the full execution_flow (every
debugging step: check_active_session, create_debug_file, symptom_gathering,
investigation_loop, resume_from_file, return_diagnosis, fix_and_verify,
request_human_verification, archive_session). Nothing here was reworded,
summarized, or deleted.

The core preamble of `gsd-debugger.md` keeps `<role>` (including the
mandatory-initial-read rule), `<debug_file_protocol>`, `<checkpoint_behavior>`,
`<structured_returns>` (the return contract), `<modes>`, `<success_criteria>`,
and a `<hard_rules_digest>` of additional verbatim MUST/CRITICAL/Do-NOT
excerpts pulled from execution_flow so they surface early in the prompt.
Those excerpts are a surfacing aid only -- the complete, authoritative
procedural context for every rule still lives here, unchanged.
