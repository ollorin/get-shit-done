...
```bash
ROADMAP=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" roadmap analyze)
```
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
```bash
ARCHIVE=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
**Commit:**
```bash
node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" init execute-phase "1")
