...
```bash
INIT=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" init verify-work "${PHASE_ARG}")
...
Commit the UAT file:
```bash
node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
