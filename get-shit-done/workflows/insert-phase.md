...
```bash
INIT=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" phase insert "${after_phase}" "${description}")
```
