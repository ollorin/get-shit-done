...
**Pattern A:** init_agent_tracking → spawn Agent(subagent_type="gsd-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
```bash
node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" phases list --type summaries --raw
...
node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" state update-progress

# Record execution metrics
node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" state record-metric \
...
node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" state record-session \
...
```bash
node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "$HOME/.claude/get-shit-done/bin/gsd-tools.js" commit "" --files .planning/codebase/*.md --amend
```
</step>
